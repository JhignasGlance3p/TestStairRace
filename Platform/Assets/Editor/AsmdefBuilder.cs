...
namespace NostraTools.Editor
{
    ...
    public class AsmdefBuilder : EditorWindow
    {
        ...
        private string gameScriptsPath = "Assets/Games/";
        ...
        private void OnEnable()
        {
            ...
            // Detect game name from selection if possible
            if (Selection.activeObject != null)
            {
                ...
                if (path.Contains("/Games/"))
                {
                    ...
                    int gamesIndex = Array.IndexOf(parts, "Games");
                    if (gamesIndex >= 0 && gamesIndex + 1 < parts.Length)
                    {
                        gameName = parts[gamesIndex + 1];
                        gameScriptsPath = $"Assets/Games/{gameName}/Scripts";
                        ValidateGameName();
                    }
                }
            }
        ...
        private void OnGUI()
        {
            ...
            GUI.backgroundColor = string.IsNullOrEmpty(gameName) ? Color.yellow : (isGameNameValid ? Color.white : new Color(1, 0.7f, 0.7f));
            ...
            if (newGameName != gameName)
            {
                ...
                // Update scripts path if game name is valid
            ...
            // Help box for game name format
            EditorGUILayout.HelpBox("Game Name is required and must follow PascalCase naming convention (e.g., 'ColorClash', 'SpaceShooter').", 
                string.IsNullOrEmpty(gameName) ? MessageType.Warning : (isGameNameValid ? MessageType.Info : MessageType.Error));
            ...
            // Create button - disabled if game name is invalid
        ...
        internal void CreateAssemblyDefinition()
        {
            try
            {
                ...
                if (!isGameNameValid)
                {
                    statusMessage = "Game name must follow PascalCase naming convention!";
                    isSuccess = false;
                    return;
                }
